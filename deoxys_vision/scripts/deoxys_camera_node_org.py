...
def main():
    ...
    if camera_info.camera_type == "k4a":
        ...
        color_cfg = EasyDict(
            enabled=node_config.use_color, img_w=640, img_h=480, img_format=rs.format.bgr8, fps=30 # we meed higher fps than policy hz    org fps=60    # img_w=1280, img_h=720, img_format=rs.format.bgr8, fps=30
        )

        # if args.use_depth:
        depth_cfg = EasyDict(
            enabled=node_config.use_depth, img_w=640, img_h=480, img_format=rs.format.z16, fps=30  # we meed higher fps than policy hz    org fps=60
        )
        # else:
        #     depth_cfg = None
