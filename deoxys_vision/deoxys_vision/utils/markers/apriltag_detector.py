...
class AprilTagDetector:
    def __init__(self, families="tag36h11"):
        ...
        self.detector = apriltag.Detector(
            families=families, quad_decimate=1,  decode_sharpening=0.25, # quad_decimate means downsample the code  quad_sigma = 0.5,
        )
        self.results = []

    def detect(self, img: np.ndarray, img_idx: int, intrinsics: dict, tag_size: int):
        ...
        detected_img = self.vis_tag(img)
        # import time
        # time = time.time()
        cv2.imwrite(f'rebar_scripts/pose_estimation/eye2hand_base/calibration_imgs/detected_{img_idx}.png', detected_img)
